...
@cormen[p. 374] consideran el algoritmo \ref{matrixmultiplication} que calcula $C = C + A \cdot B$ para tres matrices $A = (a_{ij}),\, B = (b_{ij}),\, C = (c_{ij})$ con dimensiones $p \times q,\, q \times r, \, p \times r$, respectivamente. Este algoritmo sirve para el cálculo de $A \cdot B$, para lo cual únicamente se requiere inicializar $C$ como una matriz de ceros. Este código se implementó en `Julia` mediante la función `matmul!()` (@lst-multiplicacionmatricial).
...
```{julia}
#| lst-label: lst-multiplicacionmatricial
#| lst-cap: Función de multiplicación de matrices rectangulares
...
for i in experimentos
    lab_matmul[i] =  @benchmarkable matmul!(A, B) setup = (A = $(rand(i, i)); B = $(rand(i, i)))
end
tune!(lab_matmul)
lab_matmul = run(lab_matmul, seconds = 5*60)
...
@cormen[p. 374] señalan que el algoritmo \ref{matrixmultiplication} está dominado por el término $npr$. Por lo tanto, el costo de ejecución esperado depende de $npr = n^3$ para matrices cuadradas; es decir:
...
\label{matrixmultiplication_disperse}
\begin{algorithmic}[1]
\For{$i=1$ a $p$}
    \For{$k = 1$ a $q$}
        \If{$a_{ik} \neq 0$}
...
Este código se implementó en `Julia` mediante la adición de un nuevo método a la función `matmul!()` (@lst-multiplicacionmatricial2).

```{julia}
#| lst-label: lst-multiplicacionmatricial2
#| lst-cap: Función de multiplicación de matrices rectangulares dispersas
...
Al respecto, se realizaron experimentos para ejecutar la función `matmul!()` en multiplicaciones de matrices cuadradas alatorias dispersas, que únicamente tienen el 10% de los valores diferentes de 0, de $n \times n, \, n = 100, 300, 1000$, tanto con el método señalado en el @lst-multiplicacionmatricial como con el señalado en el @lst-multiplicacionmatricial2. En la @tbl-tiempo_matmul_dispersa se muestra la mediana del tiempo de ejecución obtenido para cada uno de estos experimentos.
...
tune!(lab_matmul_d)
lab_matmul_d = run(lab_matmul_d, seconds = 5*60)
...
Como se señaló previamente, la ejecución de la iteración de la línea 4 del algoritmo \ref{matrixmultiplication_disperse} se realiza $n_1^A \cdot n$ veces; en lugar de las $n^3$ veces que se ejecuta la de la línea 3 del algoritmo \ref{matrixmultiplication}. Por lo tanto, dado que en los experimentos realizados la matriz $A$ tiene el 10% de sus elementos diferentes a cero ($n_1^A = 0.1 n^2$) se espera que los métodos implementados en el @lst-multiplicacionmatricial tengan una duración de ejecución 10 veces superior a la de los implementados en el @lst-multiplicacionmatricial2. A continuación se compara el rendimiento obtenido para estos casos, a partir de lo que se observa que no se rechazan las hipótesis nulas de que se obtuvieron estas diferencias para $n = 100,\, 300$; pero sí se obtuvieron unas diferencias un poco menos grandes para $n = 1000$.
...
@cormen[pp. 824-832] explican el procedimiento para obtener las descomposiciones $LU$ y $LUP$ de una matriz $A$ a través del método de eliminación gaussiana. A través de la descomposición $LUP$ de $A$ se obtienen una matriz triangular inferior $L$, una matriz triangular superior $U$ y una matriz de permutación $P$ tales que $PA = LU$. Para ello, @cormen plantean el algoritmo \ref{lupdecomposition}, que genera las matrices señaladas. Este código se implementó en `Julia` mediante la función `descomposicionlup()` (@lst-descomposicionlup).

\begin{algorithm}
    \caption{DESCOMPOSICIÓN-LUP($A, n$)}
    \label{lupdecomposition}
    \begin{algorithmic}[1]
...
```{julia}
#| lst-label: lst-descomposicionlup
#| lst-cap: Función de eliminación gaussiana - Descomposición LUP

function descomposicionlup(A)
    A = copy(A)
    A = convert(Matrix{Float64}, A)
    # 1. Implementa el código igual que en Cormen
    n = size(A)[1]
    @assert n == size(A)[2]
...
Al respecto, se realizaron experimentos para ejecutar la función `descomposicionlup()` (@lst-descomposicionlup) en matrices cuadradas aleatorias de $n \times n, \, n = 100, 300, 1000$. En la @tbl-tiempo_lablup se muestra la mediana del tiempo de ejecución obtenido para cada uno de estos experimentos y en la @fig-tiempo_lablup se grafican las distribuciones de dicho tiempo.

```{julia}
#| label: tbl-tiempo_lablup
#| tbl-cap: Tiempo mediano de ejecución de la `descomposicionlup()`, para $n = 100, 300, 1000$

lab_lup = BenchmarkGroup()
for i in experimentos
    lab_lup[i] =  @benchmarkable descomposicionlup(A) setup = (A = rand($i, $i) )
end
tune!(lab_lup)
lab_lup = run(lab_lup, seconds = 5*60)
...
Para resolver cada una de estas ecuaciones, se puede utilizar el algoritmo \ref{lupsolve} que utiliza la descomposición LUP de $A$ y sustituciones hacia adelante y hacia atrás para resolver el sistema de ecuaciones $Ax = b$ [@cormen, pp. 823-824].
...
Al respecto, se implementó la función `solucion_lup()` (@lst-solucionlup) en `Julia`, que resuelve un sistema $Ax = b$ a partir de la descomposición LUP de $A$ (implementada mediante `descomposicionlup()`, @lst-descomposicionlup) y el algoritmo \ref{lupsolve} de @cormen[p. 824].

```{julia}
#| lst-label: lst-solucionlup
#| lst-cap: Algoritmo para resolver el sistema $Ax = b$, basado en @cormen[p. 824] y `descomposicionlup()` (@lst-descomposicionlup)
...
Finalmente, se implementó el procedimiento para resolver $n$ sistemas lineales de la forma $AX_i = e_i$ con base en el procedimiento de `solucion_lup()` (@lst-solucionlup), a través de una función denominada `inversa()` (@lst-invertirmatriz).

```{julia}
#| lst-label: lst-invertirmatriz
#| lst-cap: Algoritmo para la inversión de una matriz - Eliminación de Gauss-Jordan
...
for i in experimentos
    lab_inv[i] =  @benchmarkable inversa(A) setup = (A = rand($i, $i))
end
tune!(lab_inv)
lab_inv = run(lab_inv, seconds = 5*60)
